...
export default {
  ...
  openImage(src) {
    ...
    if (typeof src === 'object') {
      if (src.length) {
        return PDFImage.open(src, `I${++this._imageCount}`);
      } else if (src.read) {
        return PDFImage.open(src.read, `I${++this._imageCount}`);
      } else {
        console.log(`Unknown image src: ${JSON.stringify(src).substring(0,100)}`);
      }
    }
